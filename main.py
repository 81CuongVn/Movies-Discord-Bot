...
@client.command()
async def movieshelp(ctx):
    embedVar = discord.Embed(title="Movies Help", color=0x00ff00)
    embedVar.add_field(name=".movies", value="use .movies+title of film", inline=False)
    embedVar.add_field(name=".moviesyear", value="use this to search a film with year of release. Use .movies+title of film+year of release", inline=False)
...
@client.command()                                       # wide command and processing 
async def moviesyear(ctx, arg1, arg2):
    t=arg1
    y= "&y=" + arg2
    url = "http://www.omdbapi.com/?t="+t+"&apikey=APIKEY" + y
    response = requests.request("GET", url)
    json_data = response.json()
    try:
        json_data['Error']
